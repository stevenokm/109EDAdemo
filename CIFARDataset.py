...
class CIFAR10(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        faulty_mask = torch.full_like(img_uint8, 1 << 6)
        img_uint8_faulty = torch.bitwise_or(img_uint8, faulty_mask)
        # generate repaired img (repair the faulty bit with its neighbor bit: 5)
        neighbor_mask = torch.full_like(img_uint8, 1 << 5)
        img_uint8_neighbor = torch.bitwise_and(img_uint8, neighbor_mask)
        img_uint8_neighbor = img_uint8_neighbor << 1
        ...
        sign_mask = torch.full_like(img_uint8, 1 << 7)
        img_uint8_sign = torch.bitwise_and(img_uint8, sign_mask)
        img_uint8_sign = img_uint8_sign >> 1
        img_uint8_repaired_s = np.bitwise_or(img_uint8_repaired,
                                             img_uint8_sign)
